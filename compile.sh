...
#avr-g++ $IFLAGS -g3 -mmcu=$MCU $CFLAGS spilib.c Mirf.cpp dimmer.cpp -o dimmer.elf  > dimmer.llt

CFLAGS+=" -fwhole-program -flto"
avr-c++ $IFLAGS -mmcu=$MCU $CFLAGS spilib.c Mirf.cpp dimmer.cpp -o dimmer-o.elf  > dimmer-o.llt

#/local/benejan/tmp/avr/bin/avr-g++ -mmcu=$MCU $CFLAGS spilib.o Mirf.o dimmer.o -o dimmer-o.elf

avr-objcopy -O ihex -R .eeprom -R .fuse -R .lock -R .signature dimmer-o.elf dimmer.hex
#avr-size --mcu=$MCU --format=avr dimmer.elf
avr-size --mcu=$MCU --format=avr dimmer-o.elf
#avr-objdump -h -S dimmer.elf > dimmer.lss
avr-objdump -h -S dimmer-o.elf > dimmer-o.lss